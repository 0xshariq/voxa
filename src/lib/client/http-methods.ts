import type { VoxaConfig, VoxaResponse, HttpMethod } from '../types/client-types.js';
...
export class HttpMethods {
     ...
     * TRACE request
     */
    async trace<T = any>(endpoint: string, config: VoxaConfig = {}): Promise<VoxaResponse<T>> {
        return this.request<T>('TRACE', endpoint, null, config);
    }

    /**
     * CONNECT request
     */
    async connect<T = any>(endpoint: string, data?: any, config: VoxaConfig = {}): Promise<VoxaResponse<T>> {
        return this.request<T>('CONNECT', endpoint, data, config);
    }
}
